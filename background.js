...
function drawLight() {
    ...
    // gradient.style.transform = `translate(200px , 200px )`
    ...
    for (let i = 0; i < sectBtn.length; i++) {
        sectBtn[i].addEventListener('click', function () {
          ...
          const targetX = Math.floor(document.body.clientWidth * Math.random());
          const targetY = Math.floor(document.body.clientHeight * Math.random());
          const duration = 1000; // Transition duration in milliseconds
          const startTime = performance.now();
    
          function updateLight(currentTime) {
            const elapsed = currentTime - startTime;
            const progress = Math.min(elapsed / duration, 1);
            light.x = startX + (targetX - startX) * progress;
            light.y = startY + (targetY - startY) * progress;
